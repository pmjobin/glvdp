...
layout(location = 2) uniform ivec2 plane_size;
...
const ivec2 pattern_size = ivec2(8, 8);
const uint priority_mask = 0x40;
const int max_sprite_count = 80;

ivec2 flip(ivec2 p, ivec2 size, bvec2 dir) {
	return mix(p, size - 1 - p, dir);
}

uint patternFetch(ivec2 p, uint cell) {
	...
	return palette * 16 + bitfieldExtract(strip, (p.x & (pattern_size.x - 1)) * 4 ^ 4, 4);
}

uint planeFetch(ivec2 p, int layer) {
	uint cell = texelFetch(plane_table, ivec3(p / pattern_size % plane_size, layer), 0).r;
	ivec2 q = flip(p, pattern_size, bvec2(cell & 1 << 11, cell & 1 << 12));
	return patternFetch(q, cell);
}

uint spriteFetch(ivec2 p) {
	int i = 0;
	int link = 0;
	uint color = 0;
	do {
		...
		ivec2 size = ivec2(bitfieldExtract(sprite.g, 10, 2), bitfieldExtract(sprite.g, 8, 2)) * 8 + 8;
		ivec2 q = flip(p - ivec2(sprite.ar) + 128, size, bvec2(sprite.b & 1 << 11, sprite.b & 1 << 12));
	...
	} while (i++ < max_sprite_count && link != 0 && (color & 0xF) == 0);
	return color;
}

ivec2 scrollFetch(ivec2 p, int layer) {
	const int c = pattern_size.x * 2;
	uint x = texelFetch(hscroll_table, ivec2(p.y, layer), 0).r;
	uint y = texelFetch(vscroll_table, ivec2(max((p.x + int((x + c - 1) & (c - 1)) + 1) / c - 1, 0), layer), 0).r;
	return ivec2(x, y);
}

void main() {
	ivec2 p = ivec2(gl_FragCoord.x, 224 - gl_FragCoord.y);
	ivec2 scroll_a = scrollFetch(p, 0);
	ivec2 scroll_b = scrollFetch(p, 1);
